...
object MockDispatcher : Dispatcher() {
    override fun dispatch(request: RecordedRequest): MockResponse {
        return when(request.path) {
            ...
            "/academy-tickets/academy/0" -> {
                when (request.method) {
                    "GET" -> MockResponse().setResponseCode(200).setBody(Gson().toJson(listOf(mocAcademyApp)))
                    else -> MockResponse().setResponseCode(501)
                }
            }
            "/academy-tickets/11000011" -> {
                when (request.method) {
                    "GET" -> MockResponse().setResponseCode(200).setBody(Gson().toJson(mocAcademyApp))
                    else -> MockResponse().setResponseCode(501)
                }
            }
            "/academy-tickets" -> {
